...
func DistanceToCentralPort(wires []string) int {
  instructions := GetInsgtructions(wires)
  cabinet := make(map[Pos] int)
  cross := make([]Pos, 0)

  for j:=0; j < len(instructions); j++ {
    pos := Pos{0,0}
    for _, set := range instructions[j] {
      direction := set[0]
      steps, _ := strconv.Atoi(set[1:])
      var moveFunc MoveFunc
      ...
      for i:=0; i < steps; i++ {
        ...
        if cabinet[pos] == 3 /* this only works for two lines */{
  ...
  min := int(^uint(0) >> 1)
  for _, c := range cross {
    distance := CalculateDistance(c, Pos{0,0})
    if min > distance {
      min = distance
    }
  }
...
func CalculateDistance(p Pos, i Pos) int {
  return Abs(p.x-i.x)+Abs(p.y-i.y)
}

func MoveR(p Pos) Pos {
  p.x++
  return p
}

func MoveL(p Pos) Pos {
  p.x--
  return p
}

func MoveU(p Pos) Pos {
  p.y++
  return p
}

func MoveD(p Pos) Pos {
  p.y--
  return p
}
...
func GetInsgtructions(wires []string) [][]string {
  instructions := make([][]string, len(wires))

  for i := 0; i < len(wires); i++ {
    instructions[i] = strings.Split(wires[i], ",")
  }
...
func Day03() string {
  ...
  awnser := DistanceToCentralPort(wires)
